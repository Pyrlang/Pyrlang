    ...
    async def link(self, from_pid, to_pid, local_only=False):
        ...
        if from_pid.is_local_to(self) and to_pid.is_local_to(self):
            ...
            if from_pid not in self.processes_:
                raise ValueError("from_pid not found during local-local link")
            from_process = self.processes_[from_pid]
            if to_pid not in self.processes_:
                raise ValueError("to_pid not found during local-local link")
            ...
            # check for short-circuit
            if from_pid not in self.processes_:
                raise ValueError("from_pid not found during local-remote link")
            from_process = self.processes_[from_pid]
            ...
            if to_pid not in self.processes_:
                # NOTE: This seems to generate an error reply over distribution,
                #       but I can't find documentation.
                return
            to_process = self.processes_[to_pid]
